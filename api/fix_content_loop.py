...
while True:
  ...
  try:
    article = (Article
              .select()
              .where((Article.date > hours_ago_date_time) & (Article.skip_extract == False) & ((Article.keywords.is_null(True)) | (Article.paragraphs.is_null(True))))
    ...
    try:
      article = (Article
            .select()
            .where((Article.date > hours_ago_date_time) & (Article.skip_extract == False) & ((Article.nlp_kw.is_null(True)) | (Article.summary.is_null(True))))
            .order_by(Article.date.desc())
            .get())
      try:
        extract_missing_features([article],keywords=True, nlp_kw=True, summary=True, debug=False)
